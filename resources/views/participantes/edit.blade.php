    ...
    <div class="edit-participant-container">
        ...
        <div class="form-card glassmorphism-card">
            <form id="form-participante"
                action="{{ route('participantes.update', [$capacitacion->id, $participante->id]) }}" method="POST"
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group floating-form-group">
                            <input type="text" class="form-control floating-input" name="nombre_completo"
                                id="nombre_completo" value="{{ $participante->nombre_completo }}" required>
                        ...
                        <div class="form-group floating-form-group">
                            <input type="email" class="form-control floating-input" name="correo" id="correo"
                                value="{{ $participante->correo }}" required>
                        ...
                        <div class="form-group floating-form-group">
                            <input type="text" class="form-control floating-input" name="identidad" id="identidad"
                                value="{{ $participante->identidad }}" placeholder=" " required>
                        ...
                        <div class="row g-2">
                            <div class="col-md-6 form-group floating-form-group">
                                <input type="number" class="form-control floating-input" name="edad" id="edad"
                                    value="{{ $participante->edad }}" required>
                            ...
                            <div class="col-md-6 form-group floating-form-group">
                                <select class="form-control floating-select" name="genero" id="genero" required>
                                    ...
                                    <option value="Masculino" {{ $participante->genero == 'Masculino' ? 'selected' : '' }}>
                                        Masculino</option>
                                    <option value="Femenino" {{ $participante->genero == 'Femenino' ? 'selected' : '' }}>
                                        Femenino</option>
                                    <option value="Otro" {{ $participante->genero == 'Otro' ? 'selected' : '' }}>Otro
                                    </option>
                                </select>
                        ...
                        <div class="form-group floating-form-group">
                            <select class="form-control floating-select" name="nivel_educativo" id="nivel_educativo"
                                ...
                                <option value="Universitaria Completa"
                                    {{ $participante->nivel_educativo == 'Universitaria Completa' ? 'selected' : '' }}>
                                    Universitaria Completa</option>
                                <option value="Universitaria Incompleta"
                                    {{ $participante->nivel_educativo == 'Universitaria Incompleta' ? 'selected' : '' }}>
                                    Universitaria Incompleta</option>
                                <option value="Técnico Completo"
                                    {{ $participante->nivel_educativo == 'Técnico Completo' ? 'selected' : '' }}>Técnico
                                    Completo</option>
                                <option value="Técnico Incompleto"
                                    {{ $participante->nivel_educativo == 'Técnico Incompleto' ? 'selected' : '' }}>Técnico
                                    Incompleto</option>
                                <option value="Secundaria Completa"
                                    {{ $participante->nivel_educativo == 'Secundaria Completa' ? 'selected' : '' }}>
                                    Secundaria Completa</option>
                                <option value="Secundaria Incompleta"
                                    {{ $participante->nivel_educativo == 'Secundaria Incompleta' ? 'selected' : '' }}>
                                    Secundaria Incompleta</option>
                                <option value="Primaria Completa"
                                    {{ $participante->nivel_educativo == 'Primaria Completa' ? 'selected' : '' }}>Primaria
                                    Completa</option>
                                <option value="Primaria Incompleta"
                                    {{ $participante->nivel_educativo == 'Primaria Incompleta' ? 'selected' : '' }}>
                                    Primaria Incompleta</option>
                            </select>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group floating-form-group">
                            <input type="text" class="form-control floating-input" name="ciudad" id="ciudad"
                                value="{{ $participante->ciudad }}" required>
                        ...
                        <div class="form-section payment-section">
                            ...
                            <div class="form-group file-upload-group">
                                <label class="file-upload-label">
                                    <input type="file" name="comprobante" id="comprobante"
                                        accept="image/*,application/pdf" class="file-upload-input">
                            ...
                            <div class="payment-details">
                                <div class="payment-row">
                                    ...
                                    <span class="payment-value" id="precio-display">L.
                                        {{ number_format($participante->precio, 2) }}</span>
                                    <input type="hidden" id="precio" name="precio"
                                        value="{{ $participante->precio }}">
                                </div>

                                <div class="payment-row">
                                    <span class="payment-label">ISV:</span>
                                    <span class="payment-value" id="isv-display">
                                        {{ number_format($participante->isv, 0) }}%</span>
                                    <input type="hidden" id="isv" name="isv"
                                        value="{{ $participante->isv }}">
                                </div>

                                <div class="payment-row total-row">
                                    <span class="payment-label">Total a Pagar:</span>
                                    <span class="payment-value" id="total-display">L.
                                        {{ number_format($participante->total, 2) }}</span>
                                    <input type="hidden" id="total" name="total"
                                        value="{{ $participante->total }}">
                                </div>
                            </div>
                        </div>
                    @endif
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calcularTotal() {
                ...
                @if (strtolower($capacitacion->medio) === 'pago')
                    ...
                    base = esAfiliado ? {{ $capacitacion->precio_afiliado }} :
                        {{ $capacitacion->precio_no_afiliado }};
                    impuesto = esAfiliado ? {{ $capacitacion->isv_afiliado }} :
                        {{ $capacitacion->isv_no_afiliado }};
